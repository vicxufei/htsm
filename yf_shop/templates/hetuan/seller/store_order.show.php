    ...
    <div class="ncsc-order-details">
      ...
      <div class="content">
        ...
        <dl>
          ...
          <dd><?php echo $output['order_info']['extend_order_common']['reciver_name'];?></br>
            <em class="red"><?php echo @$output['order_info']['extend_order_common']['reciver_info']['idcard_no'];?></em></br>
            ...
            <?php echo $output['order_info']['extend_order_common']['reciver_info']['dlyp'] ? '[自提服务站]' : '';?></dd>
        </dl>
      ...
      <tbody>
        ...
        <tr class="bd-line">
          ...
          <td class="tl"><dl class="goods-name">
              <dt>
                <div class="red">商品编号:<?php echo $goods['goods_barcode']; ?></div>
                <a target="_blank" href="<?php echo $goods['goods_url']; ?>">
                  <?php echo $goods['goods_name']; ?>
                </a>
                <a target="_blank" href="<?php echo urlShop('snapshot', 'index', array('rec_id' => $goods['rec_id']));?>"  class="blue ml5">[交易快照]</a>
              </dt>
          ...
          <td>
            <p>购买数量:<?php echo $goods['goods_num']; ?></p>
            <p>库存:<?php echo $goods['goods_storage']; ?></p>
            <?php if ($goods['goods_storage'] <= $goods['goods_storage_alarm'] ) {?><p><em class="red">库存报警:<?php echo $goods['goods_storage_alarm']; ?></em></p>
            <?php } ?>
          </td>
          ...
          <td class="bdl bdr" rowspan="<?php echo $output['order_info']['goods_count'];?>"><?php echo $output['order_info']['state_desc']; ?>
            ...
            <p><a href="javascript:void(0)" class="ncbtn"
                  uri="index.php?act=store_order&op=change_state&state_type=modify_price&order_sn=<?php echo $output['order_info']['order_sn']; ?>&order_id=<?php echo $output['order_info']['order_id']; ?>"
                  dialog_width="480"
                  dialog_title="<?php echo $lang['store_order_modify_price'];?>"
                  nc_type="dialog"
                  dialog_id="seller_order_adjust_fee"
                  id="order<?php echo $output['order_info']['order_id']; ?>_action_adjust_fee" />修改运费</a></p>
            <?php }?>

            <?php if (!$output['order_info']['order_printed'] && $output['order_info']['bao_sui_cang']) { ?>
              <p style="margin-bottom: 10px">
                <a href="javascript:void(0)" class="ncbtn ncbtn-lavander mt5"
                   nc_type="dialog"
                   uri="index.php?act=store_order&op=change_state&state_type=order_setprinted&order_sn=<?php echo $output['order_info']['order_sn']; ?>&order_id=<?php echo $output['order_info']['order_id']; ?>"
                   dialog_title="登记"
                   dialog_id="seller_order_setpayed_order"
                   dialog_width="400"
                   id="order<?php echo $output['order_info']['order_id']; ?>_action_cancel" />
                <i class="icon-remove-circle"></i>登记</a>
              </p>
            <?php }?>
            <!-- yefengong测试推送保税仓信息 -->
            <p style="margin-bottom: 10px">
...
$(function(){
  $('#show_shipping').on('hover',function(){
    	...
    	$.getJSON('http://www.htths.com/index.php?act=seller_center&op=get_express&e_code=<?php echo $output['order_info']['express_info']['e_code']?>&shipping_code=<?php echo $output['order_info']['shipping_code']?>&t=<?php echo random(7);?>',{},function(data){
    		if(data){
    			data = var_send+data;
