...
<div class="tabmenu">
  ...
  <a href="<?php echo urlShop('store_goods_add');?>" class="ncbtn ncbtn-mint" title="<?php echo $lang['store_goods_index_add_goods'];?>"> <?php echo $lang['store_goods_index_add_goods'];?></a>
</div>
<form method="get" action="index.php">
  <table class="search-form">
    ...
    <tr>
      <td>当前按库存排序,&nbsp;<a href="<?php echo urlShop('store_goods_online', 'goods_list');?>" class="ncbtn ncbtn-mint">按上架时间排序</a>
      </td>
      <th>供货商</th>
      ...
      <th> <select name="search_type">
          ...
          <option value="3" <?php if ($_GET['type'] == 3) {?>selected<?php }?>>条形码</option>
        </select>
      </th>
...
<table class="ncsc-default-table">
  <thead>
    <tr nc_type="table_header">
      ...
      <th class="w100"><?php echo '门店库存';?></th>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><div class="pic-thumb"><a href="<?php echo urlShop('goods', 'index', array('goods_id' => $val['goods_commonid']));?>" target="_blank"><img src="<?php echo yf_cthumb($val['goods_image']);?>" data-pic="<?php echo thumb($val, 60);?>"/> </a></div></td>
      <td class="tl"><dl class="goods-name">
          ...
          <dd class="serve"> <span class="<?php if ($val['goods_commend'] == 1) { echo 'open';}?>" title="店铺推荐商品"><i class="commend">荐</i></span> <span class="<?php if ($val['mobile_body'] != '') { echo 'open';}?>" title="手机端商品详情"><i class="icon-tablet"></i></span> <span class="" title="商品页面二维码"><i class="icon-qrcode"></i>
            <div class="QRcode"><a target="_blank" href="<?php echo goodsQRCode(array('goods_id' => $val['goods_commonid'], 'store_id' => $_SESSION['store_id']));?>">下载标签</a>
              <p><img src="<?php echo goodsQRCode(array('goods_id' => $val['goods_commonid'], 'store_id' => $_SESSION['store_id']));?>"/></p>
            </div>
            </span>
            <?php if ($val['is_fcode'] ==1) {?>
      ...
      <td><span <?php if ($val['goods_storage_alarm'] != 0 && $val['goods_storage_alarm'] > $val['goods_storage']) { echo 'style="color:red;"';}?>><?php echo $val['goods_storage'].$lang['piece']; ?></span></td>
      <td class="goods-time"><?php echo $val['chain_stock'];?></td>
