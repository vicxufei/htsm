  ...
  <table class="search-form">
    ...
    <tr>
      ...
      <td class="w100"><input type="text" class="text w80" name="buyer_name" value="<?php echo $_GET['buyer_name']; ?>" /></td>
                <th>手机号</span></th>
        <td class="w100"><input type="text" class="text w80" name="buyer_phone" value="<?php echo trim($_GET['buyer_phone']); ?>" /></td>
...
<table class="ncsc-default-table order">
  ...
  <tbody>
    ...
    <tr>
      <th colspan="20"><span class="ml10"><?php echo $lang['store_order_order_sn'].$lang['nc_colon'];?><em class="<?php if($order['state_desc']== '已取消'){echo "red";} ;?>"><?php echo $order['order_sn']; ?></em>
        ...
        <?php if ($order['chain_id'] == 1) { ?>
        <span>取货方式：门店自提(<?php echo '太仓店'; ?>) </span>
        <?php }else if($order['chain_id'] == 6) { ?>
          <span>取货方式：门店自提(<?php echo '沙溪'; ?>) </span>
        <?php }else if($order['chain_id'] == 7) { ?>
          <span>取货方式：门店自提(<?php echo '浏河'; ?>) </span>
        <?php }else if($order['chain_id'] == 8) { ?>
          <span>取货方式：门店自提(<?php echo '浏家港'; ?>) </span>
        <?php }else if($order['chain_id'] == 9) { ?>
            <span>取货方式：门店自提(<?php echo '浮桥'; ?>) </span>
        <?php }else if($order['chain_id'] == 13) { ?>
            <span>取货方式：门店自提(<?php echo '陆渡'; ?>) </span>
        <?php }else if($order['chain_id'] == 12) { ?>
            <span>取货方式：门店自提(<?php echo '直塘'; ?>) </span>
        <?php }else if($order['chain_id'] == 14) { ?>
            <span>取货方式：门店自提(<?php echo '牌楼'; ?>) </span>
        <?php } ?>
    ...
    <tr>
      <td class="bdl"></td>
      <td class="w500" style="text-align: left;">
          <p>收货人姓名:&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $order['extend_order_common']['reciver_name'];?>
              <?php if ($order['extend_order_common']['reciver_info']['idcard_no']) { ?>(身份证:<?php echo $order['extend_order_common']['reciver_info']['idcard_no'];?>)<?php } ?>
          </p>
          <p>收货人电话:&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $order['extend_order_common']['reciver_info']['phone'];?></p>
          <p>收货人地址:&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $order['extend_order_common']['reciver_info']['address'];?></p>
      </td>
      <td>
      </td>
      <td>
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>"><p class="<?php if($order['state_desc']== '已取消'){echo "red";} ;?>"><?php echo $order['state_desc']; ?>
      ...
      <td class="bdl bdr" rowspan="<?php echo $order['goods_count'];?>">
        ...
        <p><a href="javascript:void(0)" class="ncbtn ncbtn-grapefruit mt5" nc_type="dialog"
              ...
              dialog_id="seller_order_cancel_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" />
            <i class="icon-remove-circle"></i><?php echo $lang['store_order_cancel_order'];?></a></p>
        <?php } ?>

<!--      --><?php //if($order['if_store_cancel']) { ?>
<!--          <p><a href="javascript:void(0)" class="ncbtn ncbtn-lavander mt5" nc_type="dialog"-->
<!--                uri="index.php?act=store_order&op=change_state&state_type=order_setpayed&order_sn=--><?php //echo $order['order_sn']; ?><!--&order_id=--><?php //echo $order['order_id']; ?><!--"-->
<!--                dialog_title="修改订单状态为已支付"-->
<!--                dialog_id="seller_order_setpayed_order" dialog_width="400" id="order--><?php //echo $order['order_id']; ?><!--_action_cancel" />-->
<!--    <i class="icon-remove-circle"></i>设为已付款</a></p>-->
<!--      --><?php //} ?>
        
        <!-- 修改运费价格 -->
        
        <?php if ($order['if_modify_price']) { ?>
        <p><a href="javascript:void(0)" class="ncbtn-mini ncbtn-bittersweet mt10" uri="index.php?act=store_order&op=change_state&state_type=modify_price&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>" dialog_width="480" dialog_title="<?php echo $lang['store_order_modify_price'];?>" nc_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" />
        ...
        <p><a href="javascript:void(0)" class="ncbtn-mini ncbtn-bluejeansjeans mt10"
              ...
              nc_type="dialog"  dialog_id="seller_order_adjust_fee" id="order<?php echo $order['order_id']; ?>_action_adjust_fee" />
        ...
        <p><a class="ncbtn ncbtn-mint mt10"
            <?php if (!$order['bao_sui_cang'] && $order['extend_order_common']['reciver_info']['area'] != '江苏-苏州市-太仓市' && $order['extend_order_common']['reciver_info']['area'] != '江苏苏州市太仓市' && $order['extend_order_common']['reciver_info']['area'] != '江苏 苏州市 太仓市') { ?>style="background-color: red" <?php }?>
              href="index.php?act=store_deliver&op=send&order_id=<?php echo $order['order_id']; ?>"/><i class="icon-truck"></i><?php echo $lang['store_order_send'];?></a></p>
        <?php } ?>

      <?php if ($order['bao_sui_cang']) { ?>
          <em class="red">(保税仓<?php if ($order['order_printed'] == 0) { ?>,未登记<?php } ?>)</em>
      <?php } ?>

      <?php if ($order['order_printed'] == 0 && $order['chain_id'] > 0 && $order['state_desc']!= '已取消') { ?>
          <p><a href="javascript:void(0)" class="ncbtn ncbtn-lavander mt5" nc_type="dialog"
                uri="index.php?act=store_order&op=change_state&state_type=order_setprinted&order_sn=<?php echo $order['order_sn']; ?>&order_id=<?php echo $order['order_id']; ?>"
                dialog_title="修改订单状态为已支付"
                dialog_id="seller_order_setpayed_order" dialog_width="400" id="order<?php echo $order['order_id']; ?>_action_cancel" />
    <i class="icon-remove-circle"></i>打印</a></p>
      <?php } ?>
        <!-- 锁定 -->
