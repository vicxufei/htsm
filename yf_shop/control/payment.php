    ...
    private function _api_pay($order_info, $payment_info) {
        ...
        if($payment_info['payment_code'] == 'chinabank') {
            ...
            if (array_key_exists('order_list', $order_info)) {
                ...
                Tpl::output('pay_id',$order_info['pay_id']);
            } else {
...
//        $who_pay = $_POST['buyer_id'];
        ...
        //购买商品
        if ($order_type == 'real_order') {
            $result = $logic_payment->updateRealOrder($out_trade_no, $payment_info['payment_code'], $order_list, $trade_no, $who_pay);
        } elseif($order_type == 'vr_order'){
    ...
    public function returnOp(){
        ...
        if ($payment_info['payment_code'] == 'tenpay') {
            ...
            $pay_ok_url = SHOP_SITE_URL.'/index.php?controller=member_order&action=index';
            redirect($pay_ok_url);
        }
    }
    ...
    public function query_stateOp() {
        ...
        if ($_GET['pay_id'] && intval($_GET['pay_id']) > 0) {
            $info = Model('order')->getOrderPayInfo(array('pay_id'=>intval($_GET['pay_id']),'buyer_id'=>$buyer_id));
            if($info['api_pay_state'] == '1'){
                output_data('微信支付成功!');
            }
