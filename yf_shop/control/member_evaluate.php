...
class member_evaluateControl extends BaseMemberControl{
    ...
    public function addOp(){
        ...
        if (chksubmit()){
            ...
            showDialog(Language::get('member_evaluation_evaluat_success'),'index.php?act=member_order', 'succ','',2);
        } else {
            for ($i = 0, $j = count($order_goods); $i < $j; $i++) {
                $order_goods[$i]['goods_image_url'] = yf_cthumb($order_goods[$i]['goods_image'], 220);
            }
            ...
            Tpl::display2('member_evaluation');
        }
    }
