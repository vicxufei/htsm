...
class member_favorite_goodsControl extends BaseMemberControl{
    ...
    public function favoritegoodsOp(){
        ...
        if ($result){
            ...
            output_data(Language::get('favorite_collect_success','UTF-8'));
        }else{
            output_error(Language::get('favorite_collect_fail','UTF-8'));
        }
    }
    ...
    public function fglistOp(){
        ...
        if ($show == 'pic' && !empty($_GET['curpage'])) {
            //Tpl::display('favorites_goods_picshowlist.item','null_layout');
            Tpl::display2('favorites_goods');
        } else {
            //Tpl::display($show_type);
            Tpl::display2('favorites');
        }
    }
