        ...
        if (trim($_GET['keyword']) != '') {
            switch ($_GET['search_type']) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    $where['goods_barcode'] = intval($_GET['keyword']);
                    break;
            }
        }
...
//        $goods_list = $model_goods->getGoodsOnlineList2($where,'goods.goods_id,goods.goods_commonid,goods.goods_name,goods.store_id,
//        goods.goods_price,goods.goods_storage_alarm,goods.goods_storage,goods.goods_image,goods.goods_addtime,chain_stock.stock',400,'goods.goods_storage asc,chain_stock.stock asc');

      $goods_list = $model_goods->getGoodsOnlineList($where,'goods_id,goods_commonid,goods_name,store_id,
        goods_price,goods_storage_alarm,goods_storage,goods_image,goods_addtime',10,'goods_storage asc');
      $goods_id_array = array();
      foreach ( $goods_list as $k => $v ) {
        $goods_id_array[] = $v['goods_id'];
      }
      $model_chain_stock = Model('chain_stock');
      $stock_list = $model_chain_stock->getChainStockList(array('goods_id' => array('in', $goods_id_array), 'chain_id' => 1), 'goods_id,stock');

      $goods_list_new=[];
      foreach ( $goods_list as $k => $v ) {
        $goods_list_new[$k] = $v;
        foreach ( $stock_list as $kk => $vv ) {
          if($v['goods_id'] == $vv['goods_id']) {
            $goods_list_new[$k]['chain_stock'] = $vv['stock'];
          }
        }
      }


//       var_export($stock_list);

        Tpl::output('show_page', $model_goods->showpage(5));
        Tpl::output('goods_list', $goods_list_new);
        $this->profile_menu('goods_list', 'goods_list');
//        Tpl::output('show_page', $model_goods->showpage(5));
        Tpl::showpage('store_goods_list.online2');
    }
    ...
    public function edit_imageOp() {
        ...
        $model_goods = Model('goods');
        $common_list = $model_goods->getGoodsCommonInfoByID($common_id, 'store_id,goods_lock,spec_value,is_virtual');
