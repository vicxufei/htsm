     ...
     private function _tidyBrand($brand_c_list) {
        ...
        if (!empty($brand_c_list) && is_array($brand_c_list)){
            foreach ($brand_c_list as $key=>$brand_c){
                $gc_array = $brand_c['brand_initial'];
                if (empty($gc_array)) {
                    if ($brand_c['show_type'] == 1){
                        $brand_listnew[$gc_array]['text'][] = $brand_c;
                    } else {
                        $brand_listnew[$gc_array]['image'][] = $brand_c;
                    }
        ...
        return array('brand_listnew' => $brand_listnew, 'brand_r_list' => $brand_r_list);
    }
