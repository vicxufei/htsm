...
//gyf require_once BASE_PATH.'/api/payment/wxpay/log.php';

//初始化日志
//gyf $logHandler= new CLogFileHandler(BASE_DATA_PATH.'/log/wxpay/'.date('Y-m-d').'.log');
//gyf $log = Log::Init($logHandler, 15);
...
//gyf		Log::DEBUG("query:" . json_encode($result));
	...
	{
		Log::record(json_encode($data),Log::ERR);  //gyf
//gyf		Log::DEBUG("call back:" . json_encode($data));
		$notfiyOutput = array();
		...
		if ($order_type == 'r') {
		    ...
		    $result = $logic_payment->updateRealOrder($data['out_trade_no'], 'wx_saoma', $order_list, $data["transaction_id"], $data["openid"]);
		    if (!$result['state']) {
		        return false;
		    }
		    $api_pay_amount = 0;
