        ...
        $.getJSON('index.php?act=cart&op=del&cart_id=' + cart_id, function(result){
            if(result.state){
                ...
                if(result.quantity == 0){//判断购物车是否为空
                    ...
                    $('tr[cart_group="'+cart_id+'"]').remove();//移除本商品或本套装
...
function change_quantity(cart_id, input){
    ...
    $.getJSON('index.php?act=cart&op=update&cart_id=' + cart_id + '&quantity=' + _value, function(result){
        ...
        if(result.state == 'invalid'){
          ...
          $('tr[cart_group="'+cart_id+'"]').addClass('item_disabled');
...
var calc_cart_price = (function() {
    ...
    var realCalculate = function() {
        ...
        obj.children('tbody').each(function(){
            //购物车每个店铺已选择商品的总价格
            var eachTotal = 0;
            $(this).find('em[yf_id="each_goods_sum"]').each(function(){
...
$(function(){
    ...
    $('#selectAll').on('click',function(){
        if ($(this).attr('checked')) {
            ...
            if ($('input[yf_id="cart_item_check"]:checked').size() > 0) {
    ...
    $('input[yf_id="cart_item_check"]').on('click',function(){
        ...
        if (!$(this).attr('checked')) {
            ...
            // 如果都选中 则全选复选框是选中的
            var b = true;
            $('input[yf_id="cart_item_check"]').not(this).not(':disabled').each(function() {
                if (!this.checked) {
                    b = false;
                    return false;
                }
...
//        if ($(document).find('input[yf_id="cart_item_check"]:checked').size() == 0) {
