$(function(){
	...
	RuleHead += "<th class=\"w110\">首重(元/kg)<\/th>\n";
	RuleHead += "<th class=\"w110\">续重(元/kg)<\/th>\n";
	RuleHead += "<th class=\"w110\">满?元包邮<\/th>\n";
	...
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	RuleCell += "<input class=\"text w60\" type=\"text\" aria-label=\"续重\" maxlength=\"6\" autocomplete=\"off\" value=\"\" data-field=\"postageplus\" name=\"special[TRANSTYPE][CurNum][postageplus]\"><em class=\"add-on\">\n";
	RuleCell += "<i class=\"icon-renminbi\"><\/i>\n";
	RuleCell += "<\/em>\n";
	RuleCell += "<\/td>\n";
	RuleCell += "<td>\n";
	RuleCell += "<input class=\"text w60\" type=\"text\" aria-label=\"包邮\" maxlength=\"6\" autocomplete=\"off\" value=\"\" data-field=\"postagefree\" name=\"special[TRANSTYPE][CurNum][postagefree]\"><em class=\"add-on\">\n";
	RuleCell += "<i class=\"icon-renminbi\"><\/i>\n";
	RuleCell += "<\/em>\n";
	RuleCell += "<\/td>\n";
	...
	$('#dialog_batch').on('click','.J_SubmitPL',function(){
		...
		$('.tbl-except>table>tbody>tr').each(function(){
			if ($(this).find('.J_BatchField').attr('checked')){
				...
				$(this).find('input[data-field="postagefree"]').val($(obj_this).find('input[data-field="postagefree"]').val()).removeClass('input-error');
			}
		});
	    $("#dialog_batch").css('display','none');
	    $('.ks-ext-mask').css('display','none');	
