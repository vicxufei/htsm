  ...
  <div class="member-login">
    <div id="member_login">
      <ul class="tabs-nav">
        ...
        <?php if (C('sms_login') == 0){?>
        <li><a href="#mobile">手机动态码登录<i></i></a></li>
        <?php } ?>
      </ul>
      <div id="tabs_container" class="tabs-container">
        <div id="default" class="tabs-content">
          <form id="login_form" class="mb-login-form" method="post" action="<?php echo urlLogin('login', 'login');?>">
        ...
        <div id="mobile" class="tabs-content">
          <form id="post_form" method="post" class="mb-login-form" action="<?php echo urlLogin('connect_sms', 'login');?>">
            <?php Security::getToken();?>
            <input type="hidden" name="form_submit" value="ok" />
            <input name="nchash" type="hidden" value="<?php echo getNchash();?>" />
            <dl>
    ...
    <div class="qrcode-login" id="demo-form-site">
      <?php if (C('qq_isuse') == 1 || C('sina_isuse') == 1 || C('weixin_isuse') == 1){?>
      <h4><?php echo $lang['nc_otherlogintip'];?></h4>
      <?php if (C('qq_isuse') == 1){?>
      <a href="<?php echo MEMBER_SITE_URL;?>/api.php?act=toqq" title="QQ账号登录" class="qq"><i></i>QQ</a>
      <?php } ?>
      <?php if (C('sina_isuse') == 1){?>
      <a href="<?php echo MEMBER_SITE_URL;?>/api.php?act=tosina" title="新浪微博账号登录" class="sina"><i></i>新浪微博</a>
      <?php } ?>
...
$(function(){
	...
	//登录方式切换
	$('#member_login').tabulous({
		 effect: 'flip'//动画反转效果
	});	
	var div_form = '#default';
	$("#member_login .tabs-nav li a").click(function(){
