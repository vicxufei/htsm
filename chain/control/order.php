...
class orderControl extends BaseChainCenterControl{
    ...
    //http://chain.htths.com/index.php?XDEBUG_SESSION_START=PHPSTORM&act=order&op=index2&search_state_type=no&search_key_type=chain_code&keyword=
    public function indexOp() {
        ...
        if ($_GET['keyword'] != '') {
            if ($_GET['search_key_type'] == 'chain_code') {
//                $condition['chain_code'] = preg_match('/^\d{6}$/',$_GET['keyword']) ? $_GET['keyword'] : -1;
                $condition['chain_code'] = $_GET['keyword'];
            } elseif ($_GET['search_key_type'] == 'order_sn') {
                $condition['order_sn'] = preg_match('/^\d{10,20}$/',$_GET['keyword']) ? $_GET['keyword'] : -1;
            } elseif($_GET['search_key_type'] == 'buyer_phone'){
                $condition['buyer_phone'] = preg_match('/^\d{11}$/',$_GET['keyword']) ? $_GET['keyword'] : -1;
            }
        }

        if($_GET['search_key_type'] == 'goods_name'){
            $condition['goods_name'] = array('like','%'.trim($_GET['keyword']).'%');
            $order_list = $model_order->getOrderList2($condition, 20, '*', 'orders.order_id desc','', array('order_goods','order_common'));
        } else if ($_GET['search_key_type'] == 'goods_id') {
            $condition['goods_id'] = trim($_GET['keyword']);
            $order_list = $model_order->getOrderList2($condition, 200, '*', 'orders.order_id desc','', array('order_goods','order_common'));
        } else{
            $order_list = $model_order->getOrderList($condition, 20, '*', 'order_id desc','', array('order_goods','order_common'));
        }
        //页面中显示那些操作
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            $order_list[$key] = $order_info;
            if($_GET['search_key_type'] == 'goods_id' && $order_info['goods_id'] == trim($_GET['keyword'])) {
                $number += $order_info['goods_num'];
            }
        }

        Tpl::output('wei_ti_goods_num',$number);
        Tpl::output('order_list',$order_list);
        Tpl::output('show_page',$model_order->showpage());
        Tpl::showpage('order.list');
    }
