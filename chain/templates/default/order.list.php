  ...
  <table class="search-form">
    ...
    <tr>
      ...
      <th> <select name="search_key_type">
          ...
          <option value="goods_name" <?php if ($_GET['search_key_type'] == 'goods_name') {?>selected<?php }?>>商品名称</option>
          <option value="goods_id" <?php if ($_GET['search_key_type'] == 'goods_id') {?>selected<?php }?>>商品ID</option>
        </select>
      </th>
      <td class="w160">
        <input type="text" class="text w150" name="keyword" value="<?php echo $_GET['keyword']; ?>"/>
      </td>
      <?php if ($output['wei_ti_goods_num']) {?>
        <td class="w160">
          <b>未自提数量总计:<?php echo $output['wei_ti_goods_num'];?></b>
        </td>
      <?php } ?>
  ...
  <tbody>
    ...
    <td class="bdl"></td>
      ...
      <td rowspan="<?php echo $_cont;?>" class="nscs-table-handle bdl bdr"><?php if ($order_info['order_state'] != ORDER_STATE_SUCCESS) { ?>
        ...
        <?php } else { ?>
        <span>
            <a href="http://www1.htths.com/index.php?act=store_order&op=order_print&order_id=<?php echo $order_info['order_id'];?>" target="_blank" class="btn-bluejeans">
                <i class="icon-print"></i>
                <p>打印发货单</p>
            </a>
        </span>
      <span>
          <a href="javascript:void(0);" class="btn-bluejeans" onclick="javascript:ajax_form('pickup_parcel', '自提', 'index.php?act=order&op=pickup_parcel&order_id=<?php echo $order_info['order_id'];?>',900)">
              <i class="icon-truck"></i><p>自提</p>
          </a>
      </span>
        <?php } ?>
        <?php } ?></td>
