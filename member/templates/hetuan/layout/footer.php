<?php defined('ByShopWWI') or exit('Access Invalid!'); ?><?php echo getChat($layout); ?>
        ...
        <ul><?php if ($output['contract_list'])
            ...
            { ?>
                <li><a href="<?php echo $v['cti_descurl']; ?>" target="_blank"><span class="icon"> <img
                        ...
                        class="name"> <?php echo $v['cti_name']; ?> </span></a></li><?php }
            else
            { ?>
            ...
            <li class="rf">客服电话：<em class="red"><?php echo $output['setting_config']['shopwwi_phone']; ?></em> <em
                    class="rgb9"><?php echo $output['setting_config']['shopwwi_time']; ?></em></li>
        </ul>
    </div>
    <div id="faq">
        <div class="wrapper"><?php if (is_array($output['article_list']) && !empty($output['article_list']))
            ...
            { ?>
                ...
                <dl class="s<?php echo '' . $k + 1; ?>">
                    <dt><?php if (is_array($article_class['class']))
                        {
                            echo $article_class['class']['ac_name'];
                        } ?></dt><?php if (is_array($article_class['list']) && !empty($article_class['list']))
                ...
                <dl>
                    ...
                    <dd><p>错过天猫？别再错过我们！</p>
                        ...
                        <p>运维商城开启全新旅程！</p><a href="<?php echo urlShop('show_joinin', 'index'); ?>" class="btn"
        ...
        <div class="wrapper">
            <div class="screen clearfix">
                <div class="fl right-flag"><a href="http://www.shopwwi.com" target="_blank" rel="nofollow"><img
                        ...
                        href="http://www.shopwwi.com/" target="_blank" rel="nofollow"><img
                            src="<?php echo SHOP_SITE_URL; ?>/img/isc2.png"></a></div>
                <div class="fl about-us"><p><a
                            href="<?php echo SHOP_SITE_URL; ?>">返回首页</a><?php if (!empty($output['nav_list']) && is_array($output['nav_list']))
                        { ?><?php foreach ($output['nav_list'] as $nav)
                        { ?><?php if ($nav['nav_location'] == '2')
                        { ?><span>|</span> <a  <?php if ($nav['nav_new_open'])
                        { ?>target="_blank" <?php } ?>href="<?php switch ($nav['nav_type'])
                        {
                            case '0':
                                echo $nav['nav_url'];
                                break;
                    ...
                    <p>CopyRight © 2007-2016 网店交流中心 <a href="http://www.miibeian.gov.cn/" target="_blank"
