...
class member_addressControl extends BaseMemberControl{
    ...
    public function addressOp() {
        ...
        if (!empty($_GET['type'])){
            ...
            Tpl::display('member_address.edit','null_layout');
            exit();
        }
        /**
         * 判断操作类型
        ...
        Tpl::display2('member_address');
    }

    public function address_ajaxOp() {
        ...
        if (!empty($_GET['type'])){
            ...
            if (intval($_GET['id']) > 0){
                 ...
                 * 输出地址信息
                 */
                output_data('address_info',$address_info);
            }
            /**
             * 增加/修改页面输出
             */
        ...
        if (chksubmit()){
            ...
            if (intval($_POST['address_id']) > 0){
                ...
                if (!$rs){
                    output_error($lang['member_address_modify_fail']);
                }
            }else {
                $count = $address_class->getAddressCount(array('member_id'=>$_SESSION['member_id']));
                if ($count >= 10) {
        ...
        if ($del_id > 0){
            ...
            if ($rs){
                ...
                output_error(Language::get('member_address_del_fail'));
            }
        }
        $address_list = $address_class->getAddressList(array('member_id'=>$_SESSION['member_id']));
        output_data(array('address_list'=>$address_list));
