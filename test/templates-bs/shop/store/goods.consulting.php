...
$(function(){
    $('#good_consult').find('.demo').ajaxContent({
        event:'click', //mouseover
        loaderType:"img",
        loadingMsg:"<?php echo RESOURCE_SITE_URL;?>/pc/images/transparent.gif",
        target:'#good_consult'
    });
...
<div class="top" style="overflow: hidden;">
  <div class="good-cosult-tips"> <i></i>
    <p><?php echo html_entity_decode(C('consult_prompt'));?></p>
  </div>
  <div class="good-cosult-ask">
      <a href="<?php echo urlShop('goods', 'consulting_list', array('goods_id' => $_GET['goods_id']));?>#askQuestion" target="_blank" class="btn btn-grapefruit">我要提问</a></div>
</div>
<div class="good-title-nav">
  <ul id="consulting_tab">
    <li class="<?php if (intval($_GET['ctid']) == 0) {?>current<?php }?>"><a href="<?php echo urlShop('goods', 'consult', array('goods_id' => $_GET['goods_id'], 'store_id' => $_GET['store_id']));?>">全部</a></li>
    <?php if (!empty($output['consult_type'])) {?>
    <?php foreach ($output['consult_type'] as $val) {?>
    <li class="<?php if (intval($_GET['ctid']) == $val['ct_id']) {?>current<?php }?>"><a href="<?php echo urlShop('goods', 'consult', array('goods_id' => $_GET['goods_id'], 'ctid' => $val['ct_id'], 'store_id' => $_GET['store_id']));?>"><?php echo $val['ct_name'];?></a></li>
    <?php }?>
    <?php }?>
  </ul>
</div>
<div class="good-cosult-main">
  <?php if(!empty($output['consult_list'])) { ?>
  <?php foreach($output['consult_list'] as $k=>$v){ ?>
  <div class="good-cosult-list">
    <dl class="asker">
  ...
  <?php } else { ?>
  <div class="no-record"><?php echo $lang['goods_index_no_reply'];?></div>
  <?php } ?>
