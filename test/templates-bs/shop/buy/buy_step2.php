...
<form action="index.php?act=payment&op=real_order" method="POST" id="buy_form">
    ...
    <div class="receipt-info">
        <h3>
            <?php echo $output['pay']['order_remind']; ?><?php echo $output['pay']['pay_amount_online'] > 0 ? "应付金额：<strong>" . ncPriceFormat($output['pay']['pay_amount_online']) . "</strong>元" : null; ?>
        </h3>
        ...
        <dl class="cart-pd-pay">
            ...
            { ?>
                ...
                <dd>（同时勾选时，系统将优先使用充值卡，不足时扣除预存款，目前还需在线支付 <strong
                        ...
                        href="<?php echo urlMember('predeposit', 'pd_log_list'); ?>" class="predeposit">马上充值</a></dd>
    ...
    <div class="receipt-info">
        <h3>选择在线支付</h3>
        <ul class="cart-payment-list">
            <?php foreach ($output['payment_list'] as $val){ ?>
                <li payment_code="<?php echo $val['payment_code']; ?>">
                    <label for="pay_<?php echo $val['payment_code']; ?>">
                        <div for="pay_<?php echo $val['payment_id']; ?>">
                            <img src="<?php echo RESOURCE_SITE_URL ?>/pc/images/payment/<?php echo $val['payment_code']; ?>_logo.gif"/>
                        </div>
                    </label>
                </li>
            <?php } ?>
    ...
    <div class="cart-bottom">
        <a href="javascript:void(0);" id="next_button" class="pay-btn">确认支付</a>
    </div>
    <?php } ?>
</form>
...
<script type="text/javascript">
    ...
    $(function () {
        $('.cart-payment-list > li').on('click', function () {
            $('.cart-payment-list > li').removeClass('using');
