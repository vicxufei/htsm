...
function disableOtherEdit(){
	$('a[yf_id="buy_edit"]').each(function(){
...
<form method="post" id="order_form" name="order_form" action="index.php">
    ...
    <!-- 收货人,支付方式,发票信息 S -->
    <?php include template1('buy.buy_receipt');?>
    <!-- 收货人,支付方式,发票信息 E -->
    <?php if ($output['current_goods_info']['is_book']) { ?>
        <?php include template1('buy.buy_book_goods');?>
    <?php } else { ?>
        <?php include template1('buy.buy_goods_list');?>
    <?php } ?>
