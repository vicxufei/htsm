...
+function($) {
    ...
    var Util = {
        ...
        getUrlFragment: function(url) {
            ...
            var wenhaoIndex = url.indexOf('?goods_id=');
            if(wenhaoIndex === -1){
                return hashIndex === -1 ? '' : url.slice(hashIndex + 1);
            }else{
                console.log(url.slice(hashIndex + 1, wenhaoIndex));
                return hashIndex === -1 ? '' : url.slice(hashIndex + 1, wenhaoIndex);
            }
            // return hashIndex === -1 ? '' : url.slice(hashIndex + 1);
        },
        /**
     ...
     * @param {Boolean=} isPushState 是否更新路由session，默认是 false
     */
    Router.prototype.load = function(url, ignoreCache,isPushState) {
        if (ignoreCache === undefined) {
            ignoreCache = false;
        }
        if (isPushState === undefined) {
          isPushState = false;
        }
    ...
    $(function() {
        ...
        $(document).on('click', 'a', function(e) {
            ...
            if ($target.hasClass('back')) {
                ...
                var isPushState = $target.attr('data-no-history') === 'true';

                router.load(url, ignoreCache, isPushState);
            }
        });
    });
}(Zepto);
