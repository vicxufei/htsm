$(function () {
  ...
  $(document).on("pageInit", "#home", function (e, pageId, page) {
    ajax('GET','index','xianshi','',function(ret){
      renderHtml($('#xianshi'),'xianshi', ret.datas);
      var xianshi = ret.datas.xianshi;
  ...
  $(document).on("pageInit", "#goods", function (e, id, page) {
    ...
    var sort = true;
    $buttonsBar.on('click','a',function () {
      sort = false;
      var $this = $(this);
  ...
  $(document).on("pageInit", "#cart", function (e, id, page) {
    ...
    //加
    $cartList.on('click','.jQtyAdd',function () {
      var _this = $(this);
    ...
    //减
    $cartList.on('click','.jQtyMin',function () {
      var _this = $(this);
      var cartNum = _this.attr('data-num') - 0 - 1;
      var cartId = _this.data('cart_id');
      updateNum(cartId,cartNum);
    ...
    //删除
    $cartList.on('click','.jDel',function () {
      var _this = $(this);
      var cartId = _this.data('cart_id');
    ...
    //勾选/去勾选
    $cartList.on('click','.radio',function () {
      var $this = $(this);
    ...
    //全选
    $('#cart-all-checked').click(function () {
      var _this = $(this);
      var cartId = _this.data('cart_id');
  ...
  $(document).on("pageInit", "#pay", function (e, id, page) {
    ...
    $('#pay-now').click(function () {
      location.href = 'http://api.htths.com/index.php?controller=member_payment&action=pay_new&payment_code=wxpay_jsapi&pay_sn=' + orderPay.pay_sn  + '&key=' + key;
    });
  ...
  $(document).on("pageInit", "#address", function (e, id, page) {
    ...
    //点击切换收货地址
    $addressWrapper.on('click','.address-item',function () {
      var cartParm = session.get('cartParm');
    ...
    //点击编辑收货地址
    $addressWrapper.on('click','a.edit',function () {
    ...
    //点击删除收货地址
    $addressWrapper.on('click','a.del',function () {
    ...
    //点击设为默认收货地址
    $addressWrapper.on('click','a.default',function () {
      var $this = $(this);
    ...
    //添加新的收货地址
    $addressWrapper.on('click','#add-address',function () {
  ...
  $(document).on("pageInit", "#account", function (e, id, page) {
    ...
    ajax('post','member_index','index206','',function(ret){
      ...
      $('#orderState').find('a').click(function () {
  ...
  //改变订单状态的操作
  $(document).on('click','.jOrderOp',function () {
    var $this = $(this);
  ...
  //查看物流
  $(document).on('click','.jSearchDeliver',function () {
  ...
  $(document).on('opened','.popup-area', function () {
    ...
    $areaWrapper.on('click','li',function () {
      var $this = $(this);
