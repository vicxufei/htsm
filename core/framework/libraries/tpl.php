...
class Tpl{
	...
	public static function display($page_name='',$layout='',$time=2000){
		if (!defined('HT_TPL_DIR')) define('HT_TPL_DIR','default');
		self::getInstance();
		if (!empty(self::$tpl_dir)){
			$tpl_dir = self::$tpl_dir.DS;
		}
		//默认是带有布局文件
		if (empty($layout)){
			$layout = 'layout'.DS.self::$layout_file.'.html';
		}else {
			$layout = 'layout'.DS.$layout.'.html';
		}

		$layout_file = TPL_MAIN.DS.HT_TPL_DIR.DS.$layout;
		if(empty(self::$tpl_quna)){
			$tpl_file = TPL_MAIN.DS.HT_TPL_DIR.DS.$tpl_dir.$page_name.'.html';
		}else{
			$tpl_file = BASE_PATH.'/modules/'.self::$tpl_quna.'/dist/'.HT_TPL_DIR.DS.$tpl_dir.$page_name.'.html';
		}
		...
		if (file_exists($tpl_file)){
			...
			if ($layout != ''){
				if (file_exists($layout_file)){
					...
					$error = 'Tpl ERROR:'.'dist'.DS.$layout.' is not exists';
					throw_exception($error);
				}
			}else {
				include_once($tpl_file);
			...
			$error = 'Tpl ERROR:'.'dist'.DS.HT_TPL_DIR.DS.$tpl_dir.$page_name.'.html'.' is not exists';
			throw_exception($error);
		}
	}
    public static function display2($page_name=''){
        if (!defined('HT_TPL_DIR')) define('HT_TPL_DIR','default');
        self::getInstance();
        if (!empty(self::$tpl_dir)){
            $tpl_dir = self::$tpl_dir.DS;
