...
class Sms {
    ...
    public function send($mobile,$content) {
       //$shopwwi_sms_type=C('shopwwi_sms_type');
		...
		//}
		return $this->mysend_huyi($mobile,$content);
    }

	//added by yefeng
	private function mysend_huyi($mobile,$content) {
		set_time_limit(0);
//		write_file($mobile,$content."\r\n".date("Y-m-d H:i:s").'yf');

		function smsPost($mobile,$content){
			$curlPost="account=cf_313026006&password=".md5('19841128')."&mobile=".$mobile."&content=".rawurlencode($content);
