	...
	private static function control(){
		...
		if (!@include($act_file)){
		    if (C('debug')) {
		        ...
		        showMessage('Sorry！页面不存在','http://www.htths.com','html','error');
		    }
		}
...
private static function controladmin($admin){
	...
	private static function start_session(){
        ...
        ini_set('session.cookie_domain', '.htths.com');
		session_start();
	}

	public static function autoload($class){
        if(substr($class,0,5) == 'JPush'){
            ///data/wwwroot/mall.htths.com/yf_shop/api/jpush/src/JPush/Client.php
            $path = str_replace('\\', DIRECTORY_SEPARATOR, $class);
            $jPushPath = '/data/wwwroot/mall.htths.com/yf_shop/api/jpush/src/' . $path . '.php';
            if (!@include_once($jPushPath)){
	...
	private static function cp(){
		...
		if (strpos(self::CPURL,'||') !== false){
			...
			header('location: http://www.htths.com');exit();
		}else{
