...
function showMessage($msg,$url='',$show_type='html',$msg_type='succ',$is_show=1,$time=2000){
	...
	switch ($show_type){
		...
		case 'exception':
			...
			echo 'h2 { font-size: 14px; line-height: 30px; border-bottom: 1px dashed #CCC; padding-bottom: 8px;width:800px; margin: 20px 0 0 150px;}';
...
function showDialog($message = '', $url = '', $alert_type = 'error', $extrajs = '', $time = 2){
	if (empty($_GET['inajax'])){
		if ($url == 'reload') $url = '';
		showMessage($message.$extrajs,$url,'javascript',$alert_type,1,$time*1000);
	}
...
//gyf copy from mobile/framework/function/function.php start
...
//gyf copy from mobile/framework/function/function.php end
/**
 * 不显示信息直接跳转
 *
 * @param string $url
 ...
 * @return string
 */
function getMemberAvatarForID($member_avatar){
	if($member_avatar && file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_AVATAR.'/'.$member_avatar)){
        return 'http://img2.htths.com/'.ATTACH_AVATAR.'/'.$member_avatar.'?'.time();
...
function getMemberAvatarForID2($member_id){
    if(file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_AVATAR.'/avatar_'.$member_id.'.jpg')){
        return 'http://img2.htths.com/'.ATTACH_AVATAR.'/avatar_'.$member_id.'.jpg?';
        //return UPLOAD_SITE_URL.'/'.ATTACH_AVATAR.'/'.$member_avatar;
    }elseif(file_exists(BASE_UPLOAD_PATH.'/'.ATTACH_AVATAR.'/avatar_'.$member_id.'.png')){
        return 'http://img2.htths.com/'.ATTACH_AVATAR.'/avatar_'.$member_id.'.png?';
    } else{
        return UPLOAD_SITE_URL.'/'.ATTACH_COMMON.DS.C('default_user_portrait');
    }
}
...
function template1($tplpath){
	if (strpos($tplpath,'.') !== false){
		$tpltmp = explode('.',$tplpath);
		return TPL_MAIN.'/'.APP_ID.'/'.$tpltmp[0].'/'.$tpltmp[1].'.html';
	}else{
		return TPL_MAIN.'/common/'.$tplpath.'.html';
	}
}
...
function chksubmit_yf($check_token = false, $check_captcha = false, $return_type = 'alert'){
    if ($check_token && !Security::checkToken()){
        if ($return_type == 'alert'){
            showDialog('Token error!');
        }else{
    ...
    if ($check_captcha){
        if (!checkSeccode($_POST['hash'],$_POST['captchaCode'])){
            setNcCookie('seccode'.$_POST['nchash'],'',-3600);
            if ($return_type == 'alert'){
                showDialog('验证码错误!');
            }else{
...
//added by yefeng  生成controller=xxx&action=的形式
function url2($act = '', $op = '', $args = array(), $model = false, $site_url = ''){
	//伪静态文件扩展名
	$ext = '.html';
	//入口文件名
	$file = 'index.php';
...
//    }
    ...
    return url2($act, $op, $args, $rewrite_flag, SHOP_SITE_URL);
}
...
function urlMember($act = '', $op = '', $args = array()) {
    ...
    return url2($act, $op, $args, $rewrite_flag, MEMBER_SITE_URL);
}
/**
...
function urlLogin($act = '', $op = '', $args = array()) {
    return url2($act, $op, $args, false, LOGIN_SITE_URL);
}
/**
...
function yf_crul($controller,$method,$parm_name,$id){
    if($controller == 'class'){
        $url = 'http://192.168.0.151:48001/'.$controller.'/'.$method.'?'.$parm_name.'='.$id;
    }else{
        $url = 'http://192.168.0.151:48002/'.$controller.'/'.$method.'?'.$parm_name.'='.$id;
    }
