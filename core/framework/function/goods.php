...
function yf_thumb($goods = array(), $type = ''){
    if (empty($goods)){
        return UPLOAD_SITE_URL.'/'.defaultGoodsImage($type);
    }
    if (array_key_exists('apic_cover', $goods)) {
...
function yf_cthumb($file, $width = '') {
    $thumb_host = UPLOAD_SITE_URL . '/' . ATTACH_GOODS;
    return $thumb_host . '/1/'  .($width == '' ? $file : $file.'@!w'.$width);
}
function yf_cthumb2($file, $width = '') {
    $thumb_host = 'http://img1htths.img-cn-hangzhou.aliyuncs.com/shop/store/goods';
    return $thumb_host . '/1/'  .($width == '' ? $file : $file.'@!w'.$width);
}
function yf_cthumb3($file, $width = '') {
    $thumb_host = 'https://img1htths.oss-cn-hangzhou.aliyuncs.com/shop/store/goods';
    return $thumb_host . '/1/'  .$file;
}
...
function orderPaymentName($payment_code) {
    return str_replace(
            array('offline','online','alipay','tenpay','chinabank','predeposit','wxpay','wx_saoma'),
            array('货到付款','在线付款','支付宝','财付通','网银在线','站内余额支付','微信支付','微信扫码'),
            $payment_code);
}
