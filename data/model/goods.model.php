...
class goodsModel extends Model{
    ...
    public function getGoodsList2($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $count = 0) {
        $on = 'goods.goods_id = chain_stock.goods_id';
        return $this->table('goods,chain_stock')->field($field)->join('left')->on($on)->where($condition)->group($group)->order($order)->limit($limit)->page($page, $count)->select();
    }

    public function getGoodsList_yf($condition, $field = '*', $group = '',$order = '', $limit = 0, $page = 0, $count = 0) {
        $condition = $this->_getRecursiveClass($condition);
        $on = 'goods.brand_id = brand.brand_id';
        return $this->table('goods,brand')->field($field)->join('left')->on($on)->where($condition)->group($group)->order($order)->limit($limit)->select();
    }
     ...
     * @return array|null
     */
    public function getGoodsGcStoreRandList($gcId, $storeId, $notEqualGoodsId = 0, $size = 4,$fields = '*') {
        $condition = array(
        ...
        return $this->getGoodsOnlineList($condition, $fields, 0, 'rand()', $size);
    }
    ...
    public function getGoodsOnlineList2($condition, $field = '*', $page = 0, $limit = 0, $group = '', $lock = false, $count = 0) {
        $condition['goods_state']   = self::STATE1;
        $condition['goods_verify']  = self::VERIFY1;
        return $this->getGoodsList2($condition, $field, $group, $limit, $page, $count);
    }
...
//        }
    ...
    public function getGoodsDetail($goods_id) {
        ...
        $goods_info['goods_price'] = ncPriceFormat($goods_info['goods_price']);
        ...
        $goods_info['brand_info'] = Model('brand')->getBrandInfo(['brand_id'=>$goods_info['brand_id']],'brand_id,brand_name,brand_pic,brand_country_code,brand_country_en,brand_country_zh');
	   ...
	   // 新版放大镜开始
        ...
        if (!empty($image_more)) {
            foreach ($image_more as $val) {
                $goods_image[] = array(cthumb($val['goods_image'], 60, $goods_info['store_id']),yf_cthumb($val['goods_image'], 400),yf_cthumb($val['goods_image']));
                $goods_image_mobile[] = yf_cthumb($val['goods_image']);
            }
        } else {
            ...
            $goods_image_mobile[] = yf_thumb($goods_info);
        }
        // 新版结束
        ...
        //}
        if (!empty($goods_info['xianshi_info'])) {
            $goods_info['goods_price'] = ncPriceFormat($goods_info['xianshi_info']['xianshi_price']);
        }
