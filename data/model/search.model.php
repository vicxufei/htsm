...
//                 showMessage($e->getMessage(),'','html','error');
    ...
    private function _setQueryXS($condition,$order) {
        ...
        //$this->_xs_search->setScwsMulti(11)->setFuzzy()->setQuery(is_null($condition['keyword']) ? '':$condition['keyword']);
        $this->_xs_search->setFuzzy()->setQuery(is_null($condition['keyword']) ? '':$condition['keyword']);
        ...
        if (isset($condition['transport_id'])) {
            $this->_xs_search->addQueryString('transport_id'.':'.$condition['transport_id']);
        }

        if (isset($condition['brand_country_code'])) {
            $this->_xs_search->addQueryString('brand_country_code'.':'.$condition['brand_country_code']);
        }
    ...
    private function _searchXS(){
        ...
        $cates = array();
        foreach ($docs as $doc) {
            foreach (array('goods_id','goods_name','goods_jingle','brand_name','brand_id','brand_country_code','attr_id',
                    'store_id','transport_id','goods_promotion_price','goods_click','goods_salenum','goods_barcode','goods_edittime','goods_attr') as $v) {
   ...
   public function getTagCategory($keyword = '') {
        if ($keyword != '') {
            ...
            if (!$tag_list) {
                ...
                $tag_list = $model_class_tag->getTagListAll(array('gc_id','gc_tag_value'));
                wkcache('class_tag', $tag_list);
            }
            if (!empty($tag_list) && is_array($tag_list)) {
                foreach($tag_list as $key => $val) {
                    $tag_value = $val['gc_tag_value'];
    ...
    public function indexerSearch($get = array(), $pagesize) {
        ...
        if (!empty($get['c_code'])) {
            $condition['brand_country_code'] = $get['c_code'];
        }
        if (intval($get['t_id']) > 0) {
            $condition['transport_id'] = intval($get['t_id']);
        }
        ...
        if (in_array($get['key'],array('1','2','3','4'))) {
            $order = array(str_replace(array('1','2','3','4'), array('goods_salenum','goods_click','goods_promotion_price','goods_id'), $get['key'])
