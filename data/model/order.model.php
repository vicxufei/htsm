...
class orderModel extends Model {
    ...
    public function getOrderInfo($condition = array(), $extend = array(), $fields = '*', $order = '',$group = '') {
        ...
        if (in_array('order_common',$extend)) {
            if($order_info['chain_id'] >0 ){
                $order_info['chain_info'] = Model('chain')->getChainInfo(array('chain_id'=>$order_info['chain_id']),'chain_name,area_info,chain_address,chain_phone,chain_opening_hours');
            }else{
                $order_info['extend_order_common'] = $this->getOrderCommonInfo(array('order_id'=>$order_info['order_id']),'daddress_id,shipping_time,shipping_express_id,evaluation_time,order_message,deliver_explain,reciver_name,reciver_info');
        ...
        if (in_array('order_goods',$extend)) {
            //取商品列表
            $order_goods_list = $this->getOrderGoodsList(array('order_id'=>$order_info['order_id']),'rec_id,goods_id,goods_name,goods_price,goods_num,goods_image,goods_pay_price,goods_type');
     ...
     */
    public function getStoreOrderList($store_id, $order_sn, $buyer_name, $buyer_phone, $state_type, $query_start_date, $query_end_date, $skip_off, $fields = '*', $extend = array(),$chain_id = null) {
        $condition = array();
        $condition['store_id'] = $store_id;
        //if (preg_match('/^\d{10,20}$/',$order_sn)) {
        //    $condition['order_sn'] = $order_sn;
        //}
        if (preg_match('/^\d{4,20}$/',$order_sn)) {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            $transport_id= $model_goods->getGoodsList($condition,'transport_id');
            $order_info['bao_sui_cang'] = false;
            foreach ($transport_id as $k=>$v) {
                if($v['transport_id'] == 7) {
    ...
    public function getOrderList($condition, $pagesize = '', $field = '*', $order = 'order_id desc', $limit = '', $extend = array(), $master = false){
        ...
        //追加返回订单扩展表信息
        if (in_array('order_common',$extend)) {
            $order_common_list = $this->getOrderCommonList(array('order_id'=>array('in',array_keys($order_list))),'order_id,shipping_time,shipping_express_id,evaluation_time,order_message,reciver_name,reciver_info,dlyo_pickup_code');
        ...
        if (in_array('order_goods',$extend)) {
            //取商品列表
            $order_goods_list = $this->getOrderGoodsList(array('order_id'=>array('in',array_keys($order_list))),'order_id,goods_id,goods_name,goods_price,goods_num,goods_image,goods_pay_price,goods_type,goods_spec');
    ...
    public function getOrderList2($condition, $pagesize = '', $field = '*', $order = 'orders.order_id desc', $limit = '', $extend = array(), $master = false){
        $on = 'orders.order_id = order_goods.order_id';
        $list = $this->table('orders,order_goods')->field($field)->join('left')->on($on)->where($condition)->page($pagesize)->order($order)->limit($limit)->master($master)->select();
        if (empty($list)) return array();
        $order_list = array();
        foreach ($list as $order) {
            if (isset($order['order_state'])) {
     ...
     * @param unknown $condition
     */
    public function getOrderStateCancelCount($condition = array()) {
        return $this->getOrderCount($condition);
    }
    ...
    public function getOrderGoodsSum($condition, $field) {
        return $this->table('order_goods,orders')->join('inner')->on('order_goods.order_id=orders.order_id')->where($condition)->sum($field);
    }
