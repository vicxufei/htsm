...
class orderLogic {
    ...
    public function changeOrderSend($order_info, $role, $user = '', $post = array()) {
        ...
        //APP推送  yefeng
        $push_alert = $post['shipping_express_id'] == 'e1000' ? '您有订单正在配送!':'您有订单已生成快递单!';
        //if($post['shipping_express_id'] == 'e1000'){
            $inc_file = BASE_PATH.DS.'api'.DS.'jpush/yf_jpush.php';
            if(!is_file($inc_file)){
                output_error('支付接口不存在');
            }
            require($inc_file);
            $res = appPush($order_info['buyer_phone'],$push_alert, $order_info['order_id']);
//            if(!$res){
//                return callback(false,'用户未开启推送,如有必要,客服可以考虑通过其他方式通知用户!');
//            }
        //}
    ...
    public function changeOrderReceivePay($order_list, $role, $user = '', $post = array()) {
        ...
        try {
            ...
            foreach($order_list as $order_info) {
                ...
                $update_order['who_pay'] = $post['who_pay'];
                if ($post['trade_no'] != '') {
