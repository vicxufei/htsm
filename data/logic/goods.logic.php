...
class goodsLogic {
    ...
    public function updateGoods($param, $store_id, $store_state, $seller_id, $seller_name, $bind_all_gc) {
        $model_goods = Model('goods');
        ...
        // 根据参数初始化通用商品数据
        $common_array = $this->_initCommonGoodsByParam($param, $store_id, $store_state);
     ...
     * 根据参数初始化通用商品数据
     */
    private function _initCommonGoodsByParam($param, $store_id, $store_state) {
        // 分类信息
        ...
        $common_array['goods_weight']      =  $param['g_weight'];
    ...
    private function _addGoods($param, $common_id, $common_array) {
        ...
        if (is_array($param['spec'])) {
            foreach ($param['spec'] as $value) {
                ...
                $goods['goods_weight']     = $common_array['goods_weight']; //?gyf
                $goods['goods_attr']     = $common_array['goods_attr']; //?gyf
    ...
    private function _editGoods($param, $common_id, $common_array, $store_id) {
        ...
        if (is_array($param['spec'])) {
            ...
            if (!empty($goods_info)) {
                ...
                $goods['goods_attr']     = $common_array['goods_attr'];
