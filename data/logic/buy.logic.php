...
class buyLogic {
     ...
     */
    public function buyStep1($selected_area_id,$cart_id, $ifcart, $member_id, $orderdiscount = 0,$member_level = 0,$ifchain = null) {
        $store_id= 1;
        //平台店会员折扣
        $orderdiscount = 0;
        if ($orderdiscount > 0) {
        ...
        //得到购买商品信息
        if ($ifcart) {
            $result = $this->getCartList($cart_id, $member_id,$orderdiscounts);
        } else {
            $result = $this->getGoodsList($cart_id, $member_id, $store_id,$orderdiscounts);
        }

        $result['data']['selected_area_id']= $selected_area_id;
        if(!$result['state']) {
            return $result;
        }

        //得到页面所需数据：收货地址、发票、代金券、预存款、商品列表等信息
        $result = $this->getBuyStep1Data($member_id,$result['data'],$orderdiscount,$ifchain);
     ...
     */
    public function getCartList($cart_id, $member_id,$orderdiscounts = array()) {
    ...
    public function getGoodsList($cart_id, $member_id, $store_id,$orderdiscounts = array()) {
        ...
        $goods_list[0] = $store_cart_list[0] = $goods_info;
     ...
     * @return
     */
    public function getBuyStep1Data($member_id, $data, $orderdiscount = 0,$ifchain) {
        ...
        list($store_cart_list,$store_cart_total,$store_cart_weight,$store_transport_ids,$need_idcard,$allow_submit,$error_message) = $this->_logic_buy_1->calcCartList($store_cart_list,$selected_area_id,$ifchain,$member_id);

        //定义返回数组
        $result = array();
        $result['store_cart_weight'] = $store_cart_weight;
        $result['store_cart_total'] = $store_cart_total;
        $result['need_idcard'] = $need_idcard;
        $result['supported_transport_ids'] = $store_transport_ids;
        if($allow_submit){
            if($ifchain){
                $result['freight_total'] = 0;
            }else{
                $result['freight_total'] = Model('transport')->freight_calc($store_transport_ids,$selected_area_id,$store_cart_weight,$store_cart_total);
            }
        }else{
            $result['freight_total'] = 0;
        }
        $result['need2pay'] = ncPriceFormat($store_cart_total + $result['freight_total']);
        $result['allow_submit'] = $allow_submit;
        $result['error_message'] = $error_message;
        $result['store_cart_list'] = $store_cart_list;

        //返回店铺可用的代金券
       // $result['store_voucher_list'] = $this->_logic_buy_1->getStoreAvailableVoucherList($store_cart_total, $member_id);
     ...
     * @return array
     */
    public function buyStep2($post, $member_id, $member_name, $member_email, $member_mobile,$orderdiscount = 0, $member_level = 0, $device_id = 0,$ref_member_id = 0) {
    ...
    public function changeAddr($freight_hash, $city_id, $area_id, $member_id) {
        ...
        //算运费
        list($store_freight_list,$no_send_tpl_ids) = $this->_logic_buy_1->calcStoreFreight($freight_list, $area_id);
        $data = array();
    ...
    public function change_addr($store_transport_ids,$area_id,$store_cart_weight,$store_cart_total) {
        //$city_id计算运费模板,$area_id计算货到付款

        $area_id = intval($area_id);
        if ($store_transport_ids <= 0 || $area_id <= 0) return null;
    ...
    private function _createOrderStep1() {
        ...
        if (!$post['chain_id']) {
            ...
            $chain_info = Model('chain')->getChainInfo(array('chain_id'=>intval($post['chain_id'])));
            if ($chain_info) {
                ...
                $input_address_info['mob_phone'] = $this->_member_info['member_mobile'];
        ...
        //取得平台店会员折扣
        $orderdiscounts = array();
        if ($this->_member_info['orderdiscount'] > 0) {
            $own_shop_ids = Model('store')->getOwnShopIds();
            foreach ($own_shop_ids as $store_id) {
...
//            $goods_list[0] = $store_cart_list[$goods_info['store_id']][0] = $goods_info;
        ...
        list($store_cart_list3,$store_cart_total,$store_cart_weight,$store_transport_ids,$need_idcard,$allow_submit,$error_message) = $this->_logic_buy_1->calcCartList($store_cart_list,$this->_order_data['input_area_id'],$this->_order_data['input_chain_id']);
        if(!$allow_submit) {
    ...
    private function _createOrderStep3() {
        ...
        //商品金额计算(分别对每个商品/优惠套装小计、每个店铺小计)
        list($store_cart_list,$store_cart_total,$store_cart_weight,$store_transport_ids,$need_idcard,$allow_submit,$error_message) = $this->_logic_buy_1->calcCartList($store_cart_list,$input_area_id,$input_chain_id);
        ...
        //$store_final_goods_total = $store_cart_total;
        ...
        //    $freight_list = $this->_logic_buy_1->getStoreFreightList($goods_list,array_keys($cancel_calc_sid_list));
        //    list($store_freight_total,$no_send_tpl_ids) = $this->_logic_buy_1->calcStoreFreight($store_goods_weight,$freight_list,$input_area_id);
        //}
        ...
        if($input_chain_id < 1){
            $store_freight_total = Model('transport')->freight_calc($store_transport_ids,$input_area_id,$store_cart_weight,$store_cart_total);
        }else{
            $store_freight_total = 0;
        }
        //if (!$store_freight_total) {
        //    throw new Exception('抱歉，您购买的部分商品无货，请重购买');
        //}  //返回的运费可能为0
        $store_final_order_total = $store_cart_total + $store_freight_total;
        ...
        //保存数据
        $this->_order_data['store_goods_total'] = $store_cart_total;
        $this->_order_data['store_goods_weight'] = $store_cart_weight;
...
//        $this->_order_data['store_rpt_total'] = $store_rpt_total;
    ...
    private function _createOrderStep4() {
        ...
        $device_id = $this->_member_info['device_id'];
        ...
        $ref_member_id = $this->_member_info['ref_member_id'];
...
//            $promotion_sum = 0;
            ...
            $order['pay_sn'] = $pay_sn;
            $order['store_id'] = '1';
            $order['store_name'] = '太划算商城';
            $order['buyer_id'] = $member_id;
            $order['ref_member_id'] = $ref_member_id;
            ...
            $order['payment_code'] = 'wx_saoma';    //kkkkk
            $order['order_state'] = $store_pay_type_list[$store_id] == 'offline' ? ORDER_STATE_PAY : ORDER_STATE_NEW;
            $order['order_amount'] = ncPriceFormat($store_final_order_total);
            $order['shipping_fee'] = ncPriceFormat($store_freight_total);
            $order['goods_amount'] = ncPriceFormat($store_final_order_total - $store_freight_total);
            $order['order_from'] = $order_from;
            $order['order_type'] = $input_chain_id ? 3 : 1;
            ...
            $order_common['reciver_name'] = $input_chain_id ? $member_name : $reciver_name;
...
//             }
            ...
            foreach ($store_cart_list as $goods_info) {
                ...
                $order_goods[$i]['buyer_deviceid'] = $device_id;
    ...
    private function _createOrderStep6() {
        ...
        // 快递订单,减太仓门店的库存
        $input_chain_id = $input_chain_id ? $input_chain_id : 1;  //yefeng 20170219
