...
//gyf    CUR_DIALOG = ajax_form('login','登录',MEMBER_SITE_URL+'/index.php?act=login&inajax=1',360,1);
    ...
    $.getJSON(SITEURL+'/index.php?act=cart&op=del&cart_id='+cart_id+'&callback=?', function(result){
        if(result.state){
            ...
            if(result.quantity == 0){
                ...
                $('div[yf_id="appbar_total_price"]').html('');
                $('#appbar_cart_count').html('').hide();
            }else{
                ...
                obj.find('.list_detail').perfectScrollbar('destroy');
                obj.find('.list_detail').perfectScrollbar({suppressScrollX:true});
                $('div[yf_id="appbar_total_price"]').html("共计金额：<em class='goods-price'>&yen;"+result.amount+"</em>");
                $('#appbar_cart_count').html(result.quantity);
