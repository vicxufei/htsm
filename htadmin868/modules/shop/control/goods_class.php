        ...
        if (is_array($tmp_list)){
            foreach ($tmp_list as $k => $v){
                ...
                if ($v['gc_id'] == $gc_id) {
                    ...
                    $parent_pos_gcid = $v['pos_gc_id'];
                }
            }
        }
        if ($gc_id > 0){
            if ($parent_id == 0) {
                $title = '"' . $parent_name ."(".$parent_pos_gcid.")". '"的下级列表(二级)';
                $deep = 2;
            } else {
                ...
                $title = '"' . $grandparents_name . ' - ' . $parent_name . "(".$parent_pos_gcid.$gc_id.")".'"的下级列表(三级)';
                $deep = 3;
            }
            Tpl::output('deep', 3);
...
//            $this->log(L('nc_delete,goods_class_index_class') . '[ID:' . $_GET['id'] . ']',1);
            $res = yf_crul('class','delete','id',$_GET['id']);
            if($res){
    ...
    public function ajaxOp(){
        switch ($_GET['branch']){
            ...
            case 'pos_gcid':
                ...
                $where = array('gc_id' => intval($_GET['id']));
                $update_array = array();
                $update_array['pos_gc_id'] = $_GET['value'];
                $model_class->editGoodsClass($update_array, $where);
                $return = 'false';
                $res = yf_crul('class','add','id',intval($_GET['id']));
                if($res){
                    $resObj = json_decode($res,true);
                    if($resObj['flag'] == 's'){
                        $return = 'true';
...
//                exit(json_encode(array('result'=>$return)));
                exit(json_encode(array('result'=>$return,'data'=>$resObj)));
                break;
            /**
