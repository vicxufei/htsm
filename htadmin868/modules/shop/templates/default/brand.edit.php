  ...
  <form id="brand_form" method="post" name="form1" enctype="multipart/form-data">
    ...
    <div class="ncap-form-default">
      ...
      <dl class="row">
        <dt class="tit">国家</dt>
        <dd class="opt">
          <div id="country-category">
              <select class="class-select" name="brand_country">
                  <?php if(!empty($output['country_list'])){ ?>
                      <?php foreach($output['country_list'] as $k => $v){ ?>
                          <option <?php if($output['brand_array']['brand_country_code'] == $v['country_code']) {?> selected="selected" <?php }?> value="<?php echo $v['country_code'].','.$v['country_en'].','.$v['country_zh'];?>"><?php echo $v['country_zh'];?></option>
                      <?php } ?>
                  <?php } ?>
            </select>
          </div>
          <p class="notic">填写品牌所在的国家</p>
        </dd>
      </dl>
      <dl class="row">
